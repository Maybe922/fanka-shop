...
UI：**HeroUI v3**（`@heroui/react`，React Aria + Tailwind v4）。主题变量在 `src/app/globals.css` 覆盖（明亮商店风、主色赤陶橙 #E74E10）；组件已预编译带 `"use client"`，服务端页面可直接渲染，但事件回调只能在客户端组件传。给 next/link 套按钮样式用 `buttonVariants()`。
