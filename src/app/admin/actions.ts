...
// 每张卡片指向一个外链（飞书等），点卡片直接跳转。

function refreshArticle(): void {
  revalidatePath("/admin");
  revalidatePath("/"); // 首页卡片
}
...
export async function createArticle(formData: FormData): Promise<void> {
  ...
  const { title, tag, summary, linkUrl, sortOrder, isPublished } = parsed.data;
  ...
  refreshArticle();
  redirect("/admin?ok=" + encodeURIComponent("教程已创建"));
}
...
export async function updateArticle(formData: FormData): Promise<void> {
  ...
  const { id, title, tag, summary, linkUrl, sortOrder, isPublished } =
    parsed.data;
  ...
  refreshArticle();
  redirect("/admin?ok=" + encodeURIComponent("教程已保存"));
}
...
export async function deleteArticle(formData: FormData): Promise<void> {
  ...
  refreshArticle();
  redirect("/admin?ok=" + encodeURIComponent("教程已删除"));
}
